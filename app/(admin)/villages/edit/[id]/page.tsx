...
import { getVillageById } from "@/lib/services/masterService"; // Asumsi fungsi ini ada
...
/**
 * Hasilkan metadata dinamis berdasarkan desa yang diedit.
 */
export async function generateMetadata(
  ...
  return {
    title: `Ubah Desa: ${
      village?.name || "Tidak Ditemukan"
    } | Admin`,
  };
}

/**
 * Halaman Server untuk mengedit Desa.
...
export default async function EditVillagePage(
  ...
  // Ambil data desa spesifik dari server
  const village = await getVillageById(id);

  // Jika desa tidak ditemukan, tampilkan halaman 404
  if (!village) {
    notFound();
  }
  ...
  return (
    <>
      {/* Gunakan nama desa di breadcrumb */}
      <Breadcrumb pageName={`Edit Desa: ${village.name}`} showNav={false} />
      ...
      <div className="grid grid-cols-1 gap-9">
        <div className="flex flex-col gap-9">
          ...
          <div className="rounded-lg border border-stroke bg-white p-6.5 shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            {/* Berikan 'village' yang sudah di-fetch ke form.
