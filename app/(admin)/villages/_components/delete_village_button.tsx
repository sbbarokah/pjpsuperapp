...
import { deleteVillageAction } from "../actions"; // Import Server Action
...
interface DeleteVillageButtonProps {
  id: string;
  name: string;
}

export function DeleteVillageButton({ id, name }: DeleteVillageButtonProps) {
  ...
  const handleDelete = () => {
    Swal.fire({
      title: "Anda yakin?",
      text: `Anda akan menghapus desa "${name}". Aksi ini tidak dapat dibatalkan.`,
      icon: "warning",
      ...
      if (result.isConfirmed) {
        ...
        startTransition(async () => {
          const response = await deleteVillageAction(id);
