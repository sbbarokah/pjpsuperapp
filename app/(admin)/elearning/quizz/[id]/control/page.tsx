...
import { Share2, Play, Users, Trophy, ChevronRight, CheckCircle, ExternalLink, Check, Copy, Award } from "lucide-react";
...
export default function QuizControlPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 gap-4">
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          {!isFinished && (
            <button 
              ...
              className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-bold transition-all active:scale-95 border-2 ${
                copied ? 'bg-green-50 border-green-500 text-green-600' : 'bg-white border-slate-200 text-slate-600 hover:border-blue-500 hover:text-blue-600'
              }`}
            >
          ...
          {session?.status === 'waiting' ? (
            <button onClick={startQuiz} className="flex-1 md:flex-none px-8 py-3 bg-green-600 text-white rounded-2xl font-black hover:bg-green-700 shadow-xl shadow-green-600/20 flex items-center justify-center gap-2 transition-all active:scale-95">
              <Play size={20} fill="currentColor"/> MULAI KUIS
            </button>
          ) : session?.status === 'started' && (
            <button onClick={nextQuestion} className="flex-1 md:flex-none px-8 py-3 bg-blue-600 text-white rounded-2xl font-black hover:bg-blue-700 shadow-xl shadow-blue-600/20 flex items-center justify-center gap-2 transition-all active:scale-95">
      ...
      <div className={`grid grid-cols-1 ${isFinished ? 'md:grid-cols-1' : 'md:grid-cols-3'} gap-6`}>
        {!isFinished && (
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {participants.map(p => (
                  <div key={p.id} className="p-4 bg-slate-50 rounded-2xl font-black text-slate-700 border border-slate-100 text-center">{p.name}</div>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className={`bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm h-fit ${isFinished ? 'max-w-5xl mx-auto w-full' : ''}`}>
          <h2 className="text-lg font-black mb-6 flex items-center gap-2 text-slate-800">
            {isFinished ? <Award className="text-yellow-500" /> : <Trophy className="text-yellow-500" />} 
            {isFinished ? "Hasil Akhir Kuis" : "Papan Skor"}
          </h2>
          <div className="space-y-3">
            {participants.map((p, idx) => (
              <div key={p.id} className={`flex justify-between items-center p-4 rounded-2xl ${idx === 0 ? 'bg-yellow-50 border-2 border-yellow-200' : 'bg-slate-50 border border-slate-100'}`}>
                <div className="flex items-center gap-3">
