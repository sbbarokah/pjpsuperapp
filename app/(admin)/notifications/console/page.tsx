...
export default function BroadcastConsolePage() {
  ...
  // State Filter Periode & Kasus (Case 1, 2, 3, 4)
  ...
  // 1. Ambil Profil Admin yang sedang login saat mount
  useEffect(() => {
      ...
      try {
        ...
        if (user) {
          const { data: prof, error } = await supabase
            .from("profile")
            .select("user_id, role, village_id")
            .eq("user_id", user.id)
            .single();
  ...
  // Menghasilkan konten pesan dinamis berdasarkan Case pilihan
  const getBroadcastPayload = () => {
    ...
    if (activeCase === 4) {
      return {
        title: customTitle || "Pengumuman Baru 📢",
        body: customBody || "Mohon periksa pembaruan terbaru di aplikasi Anda.",
        targetPath: "/",
        externalLink: "https://pjpsuperapp.vercel.app/"
      };
    }
  ...
  const handleScanTargets = async () => {
    ...
    try {
      // Mengirim request pemindaian ke API server-side Anda secara aman
      ...
      setTargets(result);
      setSelectedTargets(new Set(result.map((t: any) => t.id))); // Pilih semua target secara default
  ...
  }, [profile, selectedMonth, selectedYear, activeCase]);

  // --- PROSES BROADCAST NOTIFIKASI MASSAL KE API + DB HISTORY ---
  const handleSendBroadcast = async () => {
    if (selectedTargets.size === 0) return;
    if (activeCase === 4 && (!customTitle.trim() || !customBody.trim())) {
      setErrorMsg("Harap isi judul dan pesan notifikasi kustom terlebih dahulu.");
      return;
    }
    ...
    for (let i = 0; i < targetArray.length; i++) {
      ...
      try {
        // Simpan riwayat notifikasi ke tabel 'user_notifications' Supabase
        await supabase
          .from("user_notifications")
          .insert({
            user_id: targetId,
            title: activeBroadcastPayload.title,
            body: activeBroadcastPayload.body,
            target_path: activeBroadcastPayload.targetPath,
            is_read: false,
            category: activeCase === 4 ? "announcement" : "reminder"
          });
        ...
        const response = await fetch("/api/notifications/send", {
          ...
          body: JSON.stringify({
            ...
            title: activeBroadcastPayload.title,
            body: activeBroadcastPayload.body,
            targetPath: activeBroadcastPayload.targetPath
          })
        });
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center bg-white dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-150 dark:border-slate-700 shadow-sm gap-4">
          <div className="flex items-center gap-4">
            <Link 
              href="/notifications"
              className="p-3 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:text-primary rounded-2xl transition-all"
            >
              <ArrowLeft size={20} />
            </Link>
            <div>
              <h1 className="text-2xl font-black text-slate-800 dark:text-white tracking-tight">Konsol Siaran Notifikasi</h1>
              <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Buat dan sebarkan notifikasi pengingat ke seluruh Admin Kelompok di bawah naungan Anda.</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  { id: 1, label: "Laporan Muslimun", desc: "Case 1" },
                  { id: 2, label: "Laporan Kehadiran", desc: "Case 2" },
                  { id: 3, label: "Laporan Penilaian", desc: "Case 3" },
                  { id: 4, label: "Pesan Bebas", desc: "Case 4 (Kustom)" }
                ].map(item => (
                  <button 
                    key={item.id}
                    onClick={() => {
                      setActiveCase(item.id as any);
                      setErrorMsg("");
                    }}
                    className={cn(
                      "p-4 rounded-2xl text-left border-2 transition-all flex flex-col justify-between h-28 w-full",
                      activeCase === item.id 
                        ? 'bg-blue-50/50 dark:bg-blue-950/20 border-blue-600' 
                        : 'border-slate-100 dark:border-slate-700 hover:border-slate-300'
                    )}
                  >
                    <span className="font-black text-[10px] text-blue-600 uppercase tracking-widest">{item.desc}</span>
                    <span className="font-bold text-xs text-slate-800 dark:text-white leading-tight">{item.label}</span>
                  </button>
                ))}
              </div>

              {/* INPUT KONDISIONAL BERDASARKAN KASUS */}
              {activeCase === 4 ? (
                /* CASE 4: FORM INPUT PESAN BEBAS / KUSTOM */
                <div className="space-y-4 p-5 bg-blue-50/30 dark:bg-blue-950/10 rounded-3xl border border-blue-100 dark:border-blue-900/30 animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="flex items-center gap-2 text-blue-800 dark:text-blue-300 font-bold text-sm mb-1">
                    <Type size={16} /> Konfigurasi Pesan Bebas (Ad-hoc)
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider ml-1">Judul Notifikasi</label>
                    <input 
                      type="text" 
                      value={customTitle}
                      onChange={(e) => setCustomTitle(e.target.value)}
                      placeholder="Contoh: Undangan Konsolidasi Bulanan 📢"
                      className="w-full p-3.5 rounded-2xl bg-white dark:bg-slate-700 dark:text-white border-2 border-slate-100 dark:border-slate-600 outline-none text-sm font-bold focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider ml-1">Isi Pesan Notifikasi</label>
                    <textarea 
                      value={customBody}
                      onChange={(e) => setCustomBody(e.target.value)}
                      placeholder="Amal sholeh kepada seluruh pengurus kelompok untuk dapat hadir pada pertemuan..."
                      rows={4}
                      className="w-full p-3.5 rounded-2xl bg-white dark:bg-slate-700 dark:text-white border-2 border-slate-100 dark:border-slate-600 outline-none text-sm font-semibold focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all resize-y"
                    />
                  </div>
                </div>
              ) : (
                /* CASE 1, 2, 3: FORM SELEKTOR PERIODE */
            ...
            {/* DAFTAR TARGET YANG DI-SCAN / DIPILIH */}
            <div className="bg-white dark:bg-slate-800 p-6 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm space-y-6">
              <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-slate-100 dark:border-slate-700 pb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-medium">
                    {activeCase === 4 
                      ? "Pilih admin kelompok mana saja di bawah desa Anda yang ingin dikirimi pesan bebas ini."
                      : "Daftar Admin Kelompok yang belum menyetor laporan pada periode di atas."}
                  </p>
                </div>
                ...
                >
                  <Loader2 size={14} className={isScanning ? "animate-spin" : "hidden"} />
                  <span>{activeCase === 4 ? "Segarkan Daftar" : "Pindai Ulang"}</span>
                </button>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                  <input 
                    ...
                    className="w-full p-2.5 pl-4 rounded-xl bg-slate-50 dark:bg-slate-700 dark:text-white border-none outline-none text-xs font-bold focus:ring-2 ring-blue-500"
                  />
                </div>
                {targets.length > 0 && (
                  <button 
              ...
              {isScanning ? (
                ...
                <div className="py-12 text-center bg-slate-50 dark:bg-slate-900/30 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-700 space-y-2">
                  ...
                  <p className="font-black text-slate-700 dark:text-white text-base">Tidak ada target</p>
                  <p className="text-xs text-slate-400 dark:text-slate-500 max-w-sm mx-auto">
                    {activeCase === 4 
                      ? "Belum ada kelompok yang terdaftar di database desa binaan Anda."
                      : "Alhamdulillah, seluruh admin kelompok terpantau sudah menyelesaikan pengisian laporan."}
                  </p>
                </div>
              ) : (
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-[2.5rem] border border-slate-100 dark:border-slate-700 shadow-sm relative overflow-hidden">
              ...
              <p className="text-xs text-slate-400 font-medium mb-8">Berikut adalah rupa notifikasi yang akan diterima oleh admin kelompok terpilih secara real-time.</p>
              ...
              <div className="mx-auto w-[260px] h-[520px] bg-slate-900 rounded-[3rem] p-3 shadow-2xl relative border-4 border-slate-800">
                ...
                <div className="w-full h-full bg-slate-950 rounded-[2.5rem] p-4 relative overflow-hidden flex flex-col justify-start pt-12 text-white">
                  ...
                  <div className="bg-slate-900/90 border border-slate-800/80 p-3 rounded-2xl backdrop-blur-md shadow-lg text-left space-y-2 relative z-10">
                    ...
                    <div className="space-y-1">
                      <h4 className="text-xs font-black text-white">{activeBroadcastPayload.title}</h4>
                    ...
                    <div className="pt-2 border-t border-slate-800 text-[8px] font-bold text-blue-400 flex items-center gap-1">
                      <span>Rute: {activeBroadcastPayload.targetPath}</span>
                    </div>
                  </div>
              ...
              <div className="mt-8 p-4 bg-slate-50 dark:bg-slate-900/40 rounded-2xl border border-slate-100 dark:border-slate-800 text-xs font-medium space-y-2">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-blue-600 dark:text-blue-400 font-bold truncate max-w-[200px]">
                    {activeBroadcastPayload.externalLink}
                  </span>
