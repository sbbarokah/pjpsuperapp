...
// Import tipe lokal dari halaman baru
import { AggregatedKbmGroupData } from "../detail/[year]/[month]/page"; 
import { KbmReportWithRelations } from "@/lib/types/report.types";

// Tipe props disesuaikan
interface KbmReportTablesProps {
  data: AggregatedKbmGroupData[];
  categories: CategoryModel[];
}

/**
 * Komponen Tabel Generik untuk Laporan Capaian, Sukses, dan Kendala
 * (Tidak berubah, sama seperti file Anda)
 */
  ...
  return (
    <div className="rounded-lg border border-stroke bg-white p-6 shadow-default dark:border-strokedark dark:bg-boxdark">
      ...
      {categories.map(category => {
        ...
        if (reportsForThisCategory.length === 0) return null;
...
// --- Komponen Utama ---
export function KbmReportTables({ data, categories }: KbmReportTablesProps) {

  // Pipihkan data (sama seperti sebelumnya)
  const allKbmReports = data.flatMap(groupData => 
    groupData.kbmReports.map(report => ({
      groupName: groupData.group.name,
      report: report
  ...
  const achievementLabels: Record<string, string> = {
    ...
    'achievement_dalil_memorization': "Hafalan Dalil", // <-- Typo diperbaiki
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* BAGIAN INI DIHAPUS DARI VERSI INI */}
